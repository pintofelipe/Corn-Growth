globals [
  ...
  base-temperature          ; Temperatura mínima para el crecimiento
  ;ideal-temperature         ; Temperatura ideal para el crecimiento
  ideal-nutrient-level      ; Nivel ideal de nutrientes
  ideal-moisture            ; Nivel ideal de humedad
]
...
plants-own [
  ...
  health                    ; Estado de salud: "healthy", "stressed" o "sick"
  growth-stage              ; Etapa de crecimiento: "seed", "seedling", "mature"
  current-growth-rate       ; Tasa de crecimiento actual
]

patches-own [
  patch-nutrient-level      ; Nivel de nutrientes en el suelo
  patch-moisture-level      ; Nivel de humedad en el suelo
  patch-temperature         ; Temperatura actual en el suelo
]
...
to setup
  ...
  set row-spacing 10
  set plant-spacing 10
  set base-temperature 10
  set ideal-temperature 25
  set ideal-nutrient-level 50
  set ideal-moisture 50  ; Ajustado para mayor precisión
...
to setup-terreno
  ask patches [
    set pcolor brown
    set patch-nutrient-level nutrient-level  ; Usar el deslizador de nutrientes
    set patch-moisture-level moisture-level  ; Usar el deslizador de humedad
    set patch-temperature temperature        ; Usar el deslizador de temperatura
  ]
end
  ...
  while [row-start <= 20] [
    ...
    while [col-start <= 20] [
      create-plants 1 [
        ...
        set current-growth-rate 0.1  ; Tasa inicial de crecimiento
      ]
      set col-start col-start + plant-spacing / 10
    ]
    set row-start row-start + row-spacing / 10
  ]
end
...
to go
  ...
  ask plants [
    ...
    let temp-current [patch-temperature] of patch-here
    let nutrients-here [patch-nutrient-level] of patch-here
    let moisture-here [patch-moisture-level] of patch-here

    ; Cálculo de Grados Día de Crecimiento (GDD)
    let GDD max list 0 (((temp-current + ideal-temperature) / 2) - base-temperature)

    ; Cálculo de estrés
    let temp-stress (1 - abs (temp-current - ideal-temperature) / ideal-temperature)
    let nutrient-stress max list 0 (1 - abs (nutrients-here - ideal-nutrient-level) / ideal-nutrient-level)
    let moisture-stress max list 0 (1 - abs (moisture-here - ideal-moisture) / ideal-moisture)

    ; Factor de crecimiento combinado
    let growth-factor GDD * temp-stress * nutrient-stress * moisture-stress

    ; Determinar salud y crecimiento
    ifelse growth-factor > 0 [
      ; Crecimiento
      set height height + (growth-factor * 0.5)
      set current-growth-rate growth-factor * 0.1

      ; Transiciones de etapa
      if height > 10 and growth-stage = "seed" [
        set growth-stage "seedling"
        set color lime
      ]
      if height > 150 and growth-stage = "seedling" [
        set growth-stage "mature"
        set color yellow
      ]

      ; Determinar estado de salud
      ifelse growth-factor > (GDD * 0.7)
        [ set health "healthy" ]
        [ set health "stressed" ]
    ] [
  ...
  ; Actualizar el suelo con variaciones más suaves
  ask patches [
    set patch-nutrient-level max list 0 (patch-nutrient-level - random-float 0.05)
    set patch-moisture-level max list 0 (patch-moisture-level - random-float 0.1)
    set patch-temperature patch-temperature + random-normal 0 0.5
  ]
...
to actualizar-plots
  ...
  ;plot mean [current-growth-rate] of plants
end
@#$#@#$#@
GRAPHICS-WINDOW
829
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "  plot mean [current-growth-rate] of plants"
